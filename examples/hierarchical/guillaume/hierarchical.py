...
class HierarchicalPathParser:
    def __init__(self, class_index : str, levels : List[str]=DEFAULT_HIERARCHY_LEVELS, as_tensor : bool=False, reversed : bool=False, *args, **kwargs):
        self.levels = levels
...
def parse_quality_control(
    ...
    paths, clss = [], []
    for path, pred, conf in zip(*[data[key] for key in ["path", "pred", "conf"]]):
