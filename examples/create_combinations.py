...
def labels_from_json(file : str):
    ...
    if "label" in data:
        ...
        labels = labels_from_taxonomy(
            create_taxonomy(
                classes,
                levels=input(f"Which levels do you want to include (default={levels_name}):") or levels_name
            )
        )
...
def labels_from_taxalist(taxa : list[str]):
    if is_taxalist(taxa):
        labels = labels_from_taxonomy(
            create_taxonomy(
                taxa, 
                levels=input("Which levels do you want to include (default=family):") or "family"
            )
        )
        labels = list(labels.values())
        levels = list(range(len(labels[0])))
