...
class XADecoderLayer(nn.Module):
    ...
    def forward(self, tgt: torch.Tensor, memory: torch.Tensor, tgt_is_causal: bool = True) -> torch.Tensor:
...
class XADecoder(BaseDecoder):
    ...
    def forward(self, tgt: torch.Tensor, memory: torch.Tensor, tgt_mask: torch.Tensor | None = None, tgt_is_causal: bool = True) -> torch.Tensor:
...
class PrefixDecoder(BaseDecoder):
    ...
    def _generate_prefix_mask(self, mem_len: int, tgt_len: int, device: torch.device) -> torch.Tensor:
        """
