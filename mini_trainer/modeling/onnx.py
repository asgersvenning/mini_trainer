...
def _copy_for_export(model, dtype):
    ...
    model = copy.deepcopy(model).cpu().to(dtype=dtype).eval()
...
def export_onnx(
    ...
    *,
    ...
    runs on CPU in its dtype; the caller's weights, modes, device and caches are
    ...
    reference = _copy_for_export(model, sample.dtype)
    ...
    with TemporaryDirectory(prefix=".onnx-export-", dir=destination.parent) as temporary:
        ...
        manifest = {
            ...
            "verification": {"provider": "CPUExecutionProvider", "rtol": rtol, "atol": atol, "cases": errors},
        }
