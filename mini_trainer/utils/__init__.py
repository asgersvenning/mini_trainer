...
from typing import (Annotated, Any, Concatenate, ParamSpec, TypeVar, Protocol,
...
class VectorizedCallable[X, **P, R](Protocol):  # noqa: D101
    @overload
    def __call__(self, x: X, /, *args: P.args, **kwargs: P.kwargs) -> R: ...
    @overload
    def __call__(self, x: Iterable[X], /, *args: P.args, **kwargs: P.kwargs) -> list[R]: ...
...
def multithread_vectorize(leave: bool = False, min_items_to_multithread: int = 32, **tqdm_kwargs: Any):
    ...
    """
    def decorator[X, R](f: Callable[Concatenate[X, P], R]) -> VectorizedCallable[X, P, R]:
