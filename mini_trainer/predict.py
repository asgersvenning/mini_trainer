...
from mini_trainer.builders import AutoEmbedder, BaseBuilder  # noqa: F401
...
from mini_trainer.utils.augmentation import debug_augmentation  # noqa: F401
...
def assign_model_from_folder(folder : str, args : dict[str, Any]):
    """Function to help assign the correct arguments when passing the result of a training run.
    """
    # Traverse and parse folder
    train_config = os.path.join(folder, "config.yaml")
    if not os.path.exists(train_config):
        raise FileNotFoundError(
            f'Model folder ({folder}) does not contain a config file (config.yaml).'
        )
    
    class_spec = os.path.join(folder, "class_spec.json")
    if not os.path.exists(class_spec):
        raise FileNotFoundError(
            f'Model folder ({folder}) does not contain a class specification file (class_spec.json).'
        )
    
    weight_folder = os.path.join(folder, "weights")
    if not os.path.exists(weight_folder):
        raise FileNotFoundError(
            f'Model folder ({folder}) does not contain a weight directory (weights).'
        )
    model_weights = os.path.join(weight_folder, "best.pt")
    if not os.path.exists(model_weights):
        raise FileNotFoundError(
            f'Model weight folder ({model_weights}) does not contain a best weight file (best.pt).'
        )
    ...
    if model_args is None:
        raise KeyError(
            f'Model config file ({train_config}) does not contain model building arguments (model_builder_kwargs).'
        )
...
def main( # noqa: D417
    ...
    # Prepare model
    # Loading the model with a lower precision leads to instable training, 
    # instead we use `torch.autocast` to facilitate mixed precision training
    ...
    with torch.inference_mode(), torch.autocast(device_type=str(device)):
        for batch in TQDM(loader, desc="Running inference", unit="batch"):
            ...
            #     if not pred_is_list:
            #         loss = [
            #           criterion(p.unsqueeze(0), l.unsqueeze(0).unsqueeze(0))
            #           for p, l in zip(pred, labs.to(device))
            #         ]
            #     else:
            #        loss = [
            #           criterion([pi.unsqueeze(0) for pi in p], l.unsqueeze(0)) 
            #           for p, l in zip(zip(*pred), labs.to(device))
            #        ]
            #     loss = [
...
def cli(description="Classify images with a trained model", **extra_kwargs): # noqa: D103
    ...
    mod_args.add_argument(
        ...
        default=None, required=False,
        help='Path to folder created when training a model.\n'
