...
function drawProjection(refreshThumbnails=true) {
 resizeProjectionRaster();
 const pixels=1200/(projectionCanvas.getBoundingClientRect().width||1200);
 ...
 ctx.lineWidth=pixels;ctx.strokeStyle='#dce3e9';ctx.fillStyle='#52687b';ctx.font=`${12*pixels}px system-ui`;
 ...
 for(let i=0;i<projectionHits.length;i++){
  ...
  ctx.fillStyle='#7c90a34d';ctx.beginPath();ctx.arc(x,y,2*pixels,0,2*Math.PI);ctx.fill();
 }
 ...
 for(const i of projected){if(original.has(i))continue;const [x,y]=projectionHits[i];ctx.strokeStyle='#c87719';ctx.lineWidth=1.8*pixels;ctx.beginPath();ctx.arc(x,y,5*pixels,0,2*Math.PI);ctx.stroke();}
 ...
 if(refreshThumbnails!==false&&typeof scheduleMapThumbnails==='function')scheduleMapThumbnails();
}
...
function resizeProjectionRaster(){
 const rect=projectionCanvas.getBoundingClientRect();if(!rect.width||!rect.height)return;
 const dpr=window.devicePixelRatio||1,w=Math.round(rect.width*dpr),h=Math.round(rect.height*dpr);
 if(projectionCanvas.width!==w||projectionCanvas.height!==h){projectionCanvas.width=w;projectionCanvas.height=h;}
 projectionCanvas.getContext('2d').setTransform(w/1200,0,0,h/projectionHeight,0,0);
}
window.addEventListener('resize',()=>{if(projectionCanvas.getBoundingClientRect().width)drawProjection();});
