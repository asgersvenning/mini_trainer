...
| Calibrated TensorRT inference | Both representative heads execute 170 INT8 convolutions and two INT8 head GEMMs; full 912-image Blair evaluation | Significant speed/runtime-memory benefit against FP16; target desktop/Spark results; maintained production preparation/inspection workflow |
...
1. **Make the latest experiments reproducible from a clean checkout.** Promote the
   useful preparation, engine inspection, paired inference and metric-evaluation
   probes into maintained developer commands with explicit optional environments.
