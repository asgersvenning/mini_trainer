---
name: hf-cli
description: "Use the Hugging Face hf CLI for Hub authentication, model and dataset repositories, downloads, uploads, tags, and Spaces. Use for Hugging Face integration or publication tasks, not unrelated training or ML work."
---
...
revision `80f9fa530e46f4ae642fcb9e1725bad0e1979395`, under the included
[Apache-2.0 license](LICENSE). Local changes narrow the trigger and replace the
installation preamble with repository-specific guidance; the command reference is
unchanged.

This reference was generated for `huggingface_hub 2.0.0`; publication workflows
currently use `>=1.19,<2`. Check the selected environment's `hf version` and
`hf <command> --help` before relying on newer flags. The skill does not install
or upgrade the CLI. Use an existing environment's `hf`, or an isolated
`uvx --from 'huggingface_hub==2.0.0' hf` invocation for this reference version.
Do not synchronize or upgrade the training environment to obtain CLI tools.

Follow [the publication handoff](../../../dev/releases/mambo_v3/publication.md)
for release actions, authorization and trusted publishing. Prefer existing OIDC
credentials in Actions; never print `hf auth token` output or commit credentials.
Installing this skill does not authorize publication or paid resources.
