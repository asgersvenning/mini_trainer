...
def test_wandb_logger_update_and_step():
    ...
    with patch.object(wandb_module, "wandb", mock_wandb):
        ...
        logger.step()
        mock_wandb.log.assert_called_once_with({"loss/main": 1.5}, step=0)
...
def test_wandb_logger_add_figure():
    ...
    with patch.object(wandb_module, "wandb", mock_wandb):
        ...
        mock_wandb.log.assert_called_once_with({"my_plot/main": mock_wandb.Image.return_value, "epoch": 1}, step=0)
